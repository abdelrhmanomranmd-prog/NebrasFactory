...
(function (global) {
        ...
        function next() {
            ...
            loadBundle(name).catch(function () { /* soft */ }).then(function () {
                if (typeof requestIdleCallback === 'function') {
                    requestIdleCallback(next, { timeout: 8000 });
                } else {
                    setTimeout(next, 400);
                }
            });
    ...
    function schedulePrefetch() {
        function run() {
            ...
            prefetchNebrasAdminDepts();
        }
        var delayMs = 10000;
        function afterIntro() {
            setTimeout(run, delayMs);
        }
        if (document.body && document.body.classList.contains('nebras-ready')) {
            afterIntro();
        } else {
            window.addEventListener('nebras-intro-finished', afterIntro, { once: true });
