path = r'c:\Users\abdel\OneDrive\Desktop\NebrasFactory\js\nebras-media-admin.js'
JS = r'''
        ...
        function openNebrasMediaHub(options) {
            ...
            return new Promise(function(resolve) {
                ...
                if (hintEl) hintEl.textContent = options.hint || 'صور أو PDF — تُرفع إلى Supabase.';
            ...
            openNebrasMediaFilePicker(function(file) {
                ...
                uploadNebrasMediaFile(file).then(function(url) {
                    ...
                    if (url) {
                        ...
                        nebrasMediaHubSetStatus('فشل الرفع. جرّب «مسار / رابط» أو سكربت التخزين.', true);
                    }
                });
            }, accept);
        }
        ...
        function nebrasMediaHubConfirm() {
            ...
            if (isManual) {
                ...
                if (!url) {
                    nebrasMediaHubSetStatus('ارفع ملفاً أو استخدم المسار اليدوي.', true);
                    return;
                }
            }
        ...
        async function nebrasViconEditorAddAlbum() {
            const url = await openNebrasMediaHub({ label: 'صورة للألبوم', accept: 'image/jpeg,image/png,image/webp,image/gif,image/avif' });
            if (url) {
        ...
        async function nebrasViconEditorAddDocument() {
            ...
            const url = await openNebrasMediaHub({ label: 'وثيقة / PDF', accept: 'image/jpeg,image/png,image/webp,application/pdf' });
            if (url) {
        ...
        function saveVisitorIconEditor() {
            ...
            const linkedProductId = document.getElementById('nebras-vicon-linked-product').value.trim();
            const backgroundImage = document.getElementById('nebras-vicon-background').value.trim();
            if (!titleAr || !iconClass || !target) {
        ...
        function previewVisitorIconEditor() {
            ...
            else alert('احفظ الأيقونة أولاً.');
        }
        ...
        function initNebrasMediaAdminOverrides() {
            ...
            window.pickMediaAlbumInteractive = async function(existingAlbum) {
                ...
                while (true) {
                    const url = await openNebrasMediaHub({ label: 'صورة للأlbوم (' + (album.length + 1) + ')', accept: 'image/jpeg,image/png,image/webp,image/gif,image/avif' });
                    if (!url) break;
                    album.push(url);
            ...
            window.pickMediaAlbumInteractive = async function(existingAlbum) {
                let album = (existingAlbum || []).slice();
                while (true) {
                    const url = await openNebrasMediaHub({ label: 'صورة للألبوم (' + (album.length + 1) + ')', accept: 'image/jpeg,image/png,image/webp,image/gif,image/avif' });
                    if (!url) break;
                    album.push(url);
                    if (!confirm('إضافة صورة أخرى للألبوم؟')) break;
                }
            ...
            const origAdd = window.addVisitorIcon;
