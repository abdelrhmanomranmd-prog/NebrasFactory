#!/usr/bin/env python3
"""Verify hrws257 live: organized deploy, SVG studio, 360°, clean structure."""
import re
import sys
import time
...
def get(path):
    req = urllib.request.Request(
        SITE + path,
        headers={'User-Agent': 'NebrasVerify/257', 'Cache-Control': 'no-cache'},
    )
...
def main():
    ...
    for attempt in range(15):
        ...
        try:
            ...
            print('JS', st3, 'CSS', st4)

            flags = {
                'deploy': ver == TARGET,
                'health': (up.group(1) if up else None) == TARGET,
                'svg_studio': 'DOOR_DESIGNER_LIVE_USE_PHOTO_PRESETS = false' in js,
                'studio_live': "designCanvasMode: 'studio-live'" in js,
                'rot_360': 'const ROT_MIN = -180' in js and 'const ROT_MAX = 180' in js,
                'ghost_guard_css': ':not(.wpc-door-stage--photo-preset) .wpc-door-svg-overlay' in css,
            ...
            for path in [
                '/js/nebras-platform.js?v=' + jv,
                '/css/12-door-designer.css?v=' + jv,
            ...
            if not issues:
                print('RESULT: PASS — hrws257 live')
                return 0
