#!/usr/bin/env python3
"""Verify hrws316 — photoreal transom photos, no CSS overlay box."""
import sys
...
def main():
    html = fetch_text('/')
    js = fetch_text('/js/nebras-platform.js?v=hrws316')
    css = fetch_text('/css/12-door-designer.css?v=hrws316')

    transom_resolve = js.split('function resolveDoorDesignerPhotoPreset')[1].split('function clearDoorDesignerPhotoPreset')[0]

    checks = {
        'deploy_hrws316': 'data-nebras-deploy="hrws316"' in html,
        'cache_316': "DOOR_PHOTO_PRESET_CACHE = '316'" in js,
        'baked_transom_first': "DOOR_PHOTO_PRESET_MAP[transomKey]" in transom_resolve.split('plainKey')[0] or transom_resolve.find("DOOR_PHOTO_PRESET_MAP[transomKey]") < transom_resolve.find("mode: 'composite-transom'"),
